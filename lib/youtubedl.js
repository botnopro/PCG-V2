...
const ogmp3 = {
  ...
  download: async function(link, format, type = 'video') {
    if (!link) return { status: false, code: 400, error: "Missing link." };
    if (!this.isUrl(link)) return { status: false, code: 400, error: "Invalid YouTube link." };
    if (type !== 'video' && type !== 'audio') return { status: false, code: 400, error: "Type must be 'video' or 'audio'." };
    if (!format) format = type === 'audio' ? this.default_fmt.audio : this.default_fmt.video;
    const valid_fmt = type === 'audio' ? this.formats.audio : this.formats.video;
    if (!valid_fmt.includes(format)) return { status: false, code: 400, error: `Invalid format ${format}. Choose from: ${valid_fmt.join(', ')}.` };

    const id = this.youtube(link);
    if (!id) return { status: false, code: 400, error: "Could not extract video ID." };
    ...
    try {
      ...
      while (retries < maxRetries) {
        ...
        const resx = await this.request(`/${c}/init/${this.utils.enc_url(link)}/${d}/`, req);
        if (!resx.status) {
        ...
        if (data.le) return { status: false, code: 400, error: "Video duration too long (max 3 hours)." };
        if (data.i === "blacklisted") {
          const limit = this.restrictedTimezones.has(new Date().getTimezoneOffset().toString()) ? 5 : 100;
          return { status: false, code: 429, error: `Daily download limit (${limit}) reached.` };
        }
        if (data.e || data.i === "invalid") return { status: false, code: 400, error: "Video does not exist or is restricted." };
        if (data.s === "C") {
          return {
            status: true,
            code: 200,
        ...
        const prod = await this.checkProgress(data);
        if (prod && prod.s === "C") {
          return {
            status: true,
            code: 200,
            result: {
              ...
              format: format,
              thumbnail: `https://i.ytimg.com/vi/${id}/maxresdefault.jpg`,
              download: `${this.api.base}/${this.utils.hash()}/download/${this.utils.encoded(prod.i)}/${this.utils.hash()}/`,
              id: id,
              quality: format
            }
          };
      ...
      return { status: false, code: 500, error: "Failed after multiple attempts." };
    } catch (error) {
